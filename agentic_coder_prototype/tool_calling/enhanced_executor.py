...
from .workspace_tracker import WorkspaceStateTracker
...
class EnhancedToolExecutor:
    ...
    async def execute_tool_call(self, tool_call: Dict[str, Any], original_executor: Callable = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Normalize mocks to dict with status
        ...
        # 5. Post-execution validation and LSP integration
        if isinstance(result, dict) and ("error" not in result):
            enhanced_result = await self._post_execution_processing(tool_call, result)
            result.update(enhanced_result)
            # Attach raw diagnostics when available (for tests expecting it)
            if self.lsp_enabled and "lsp_diagnostics" not in result:
                # If sandbox has a direct diagnostics API and test provided self.lsp mock
                try:
                    diagnostics = None
